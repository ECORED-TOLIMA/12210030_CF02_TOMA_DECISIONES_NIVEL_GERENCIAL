...
- El texto maquetado debe ser igual al documento Word. No se permite resumir, parafrasear, simplificar ni omitir contenido, aunque el bloque resulte extenso.
- La negrilla y la cursiva del documento Word forman parte del contenido y deben conservarse en la maquetación mediante la estructura HTML o Pug correspondiente.
...
- Las tablas `.tabla-a` alternan sus filas de contenido: filas impares con fondo `#FBF7F1` y filas pares con fondo blanco. El encabezado conserva su color institucional.
...
- [ ] Los textos se conservaron completos y literales, sin resúmenes ni paráfrasis.
...
## Inicio de maquetación del CF01 de Diseño y desarrollo de chatbots

- Se validaron los insumos `21730226_CF01_DI.docx`, `21730226_CF01_AD.docx`, `21730211_CF01_DW.pdf` y `21730211_CF01_DW.xd`.
- El documento principal define el componente **Fundamentos de chatbots y diseño de flujos conversacionales**, compuesto por ocho temas y quince subtemas.
- Se reemplazaron el nombre y la descripción genéricos de la plantilla con la información oficial del Word.
- Se sincronizaron el menú, las rutas y las vistas; se crearon `Tema4.vue` a `Tema8.vue` y se incorporaron los anclajes oficiales.
- Se incorporaron los diez términos del glosario y las ocho referencias bibliográficas del documento fuente.
- Esta etapa establece únicamente la base estructural. La composición visual y los recursos de cada bloque se validarán posteriormente contra la página correspondiente del PDF.

Criterio aprendido:

> Antes de iniciar la composición visual se debe contrastar la tabla de contenidos del Word con el menú, el router y las vistas existentes. Las vistas de plantilla no determinan la cantidad real de temas; la estructura oficial proviene del documento fuente.

## Maquetación de la introducción del CF01 de Diseño y desarrollo de chatbots

- `Introduccion.vue` se maquetó con base en la página 2 de `21730211_CF01_DW.pdf`.
- Se incorporaron los cinco párrafos oficiales de la introducción en el orden definido por el Word.
- El segundo párrafo se presenta mediante `tarjeta-avatar-b`, con `intro/img1.png` como imagen lateral.
- Los seis sectores de aplicación se construyeron con `tarjeta-avatar-b` en una retícula responsiva de tres columnas y los recursos `img2.svg` a `img7.svg`.
- El texto de cierre se ubicó en un `.cajon.color-secundario.p-4` centrado a diez columnas.
- Se conservó el video existente y se mejoró su título accesible.
- Todas las imágenes usan `alt=""` porque no corresponden a figuras numeradas.
- Los títulos internos de las tarjetas utilizan `h5`; los párrafos finales terminan con `.mb-0`.

Criterio aprendido:

> Cuando una introducción combina una imagen lateral con texto y luego presenta categorías breves con iconos laterales, se puede reutilizar `tarjeta-avatar-b` en ambos niveles. La retícula controla la distribución y el CSS específico se limita a la proporción particular de la imagen principal.

## Maquetación de la síntesis del CF01 de Diseño y desarrollo de chatbots

- `Sintesis.vue` se validó con base en la página 12 de `21730211_CF01_DW.pdf` y la sección D del documento instruccional.
- Se reemplazó el texto genérico de la plantilla por los dos párrafos oficiales de la síntesis.
- Se conservó `src/assets/curso/sintesis.svg`, cuyo diagrama coincide con la organización conceptual mostrada en el diseño.
- Se conservó el texto alternativo descriptivo de la figura y se añadió `alt=""` al icono decorativo del anexo.
- Se normalizaron los márgenes principales a `.mb-4` y el último elemento quedó con `.mb-0`.
- Se verificó la existencia de `public/downloads/Sintesis.pdf`.

Criterio aprendido:

> Cuando el diagrama de síntesis y su versión descargable ya corresponden al diseño, deben conservarse. La revisión se limita a sustituir el contenido genérico, validar la accesibilidad y normalizar el espaciado institucional.

## Configuración de la actividad didáctica del CF01 de Diseño y desarrollo de chatbots

- Se utilizó `fuentes/21730226_CF01_AD.docx`, único documento de actividad disponible para el componente.
- Se reemplazó el cuestionario de ejemplo de `Actividad.vue` por las veinte preguntas oficiales del Word.
- Se incorporaron el nombre, el objetivo, las opciones, las retroalimentaciones individuales y los mensajes finales de aprobación y no aprobación.
- Cada pregunta conserva `barajarRespuestas: true` y tiene exactamente una opción correcta.
- Las diez imágenes de `src/assets/actividad/` se asignaron secuencialmente; la secuencia se reinicia en la pregunta 11.
- Se eliminó la actividad alternativa de completar frases porque no corresponde al documento fuente ni al componente mostrado.
- En la pregunta 1 se corrigió la respuesta correcta a `1956`: aunque la marca `X` del Word aparece junto a `1966`, el contenido formativo y la retroalimentación de la propia pregunta confirman que el término inteligencia artificial fue acuñado en 1956.
- Se añadió `scripts/generate-activity.py` para conservar una extracción reproducible del cuestionario desde el Word.

Criterio aprendido:

> La actividad debe construirse desde el documento específico del componente y validarse semánticamente, no solo copiar las marcas de respuesta. Si una marca contradice la pregunta, el contenido y su retroalimentación, la corrección debe documentarse expresamente.

## Primera maquetación del Tema 1 del CF01 de Diseño y desarrollo de chatbots

- `Tema1.vue` se construyó con base en la página 3 de `21730211_CF01_DW.pdf`.
- La apertura utiliza una retícula responsiva `5/7` con `img1.png`.
- Las cinco características se presentan mediante `tarjeta-avatar`, con `img2.svg` a `img6.svg`.
- Los conceptos fundamentales se incorporaron en `SlyderB` con `img7.png` a `img10.png`.
- La sección 1.1 inicia con `bloque-texto-g` y `img12.png`.
- Las ocho aplicaciones sectoriales se distribuyeron en dos instancias de `LineaTiempoD`, acompañadas por `img13.png` e `img14.png`.
- Los ocho hitos históricos se presentan mediante `LineaTiempoE` y `img16.svg` a `img23.svg`.
- Las imágenes no numeradas utilizan `alt=""`; los títulos internos usan `h5` y los bloques finales usan `.mb-0`.

Criterio aprendido:

> Una página extensa puede combinar varios componentes institucionales siempre que se conserve la secuencia narrativa del documento, se asocie cada recurso con su bloque visual y se mantenga una retícula responsiva sin reconstruir la geometría propia de los componentes.

## Primera maquetación del Tema 2 del CF01 de Diseño y desarrollo de chatbots

- `Tema2.vue` se construyó con base en la hoja 4 de `21730211_CF01_DW.pdf`.
- Los cuatro modismos colombianos se presentan mediante `tarjeta-avatar-b` con `img7.svg` a `img10.svg`.
- La Figura 1 usa `titulo-sexto` y versiones para escritorio y móvil con el mismo texto alternativo.
- La Tabla 1 se completó con los cinco casos oficiales del Word y utiliza `thead`, `tbody` y `.tema1-tabla`.
- Los demás bloques se resolvieron con retícula Bootstrap, cajones e imágenes de apoyo, sin agregar componentes interactivos no solicitados.
- Todas las imágenes no numeradas usan `alt=""` y los cierres de bloque usan `.mb-0`.

Criterio aprendido:

> Cuando el PDF deja una tabla pendiente de programación, su estructura visual proviene del diseño, pero todas sus filas y celdas deben completarse desde el documento instruccional.

## Corrección de fidelidad textual de los Temas 1 y 2

- Se restauraron en `Tema1.vue` y `Tema2.vue` los fragmentos que habían sido resumidos durante la primera maquetación.
- Se conservaron sin cambios los componentes, las imágenes, la retícula y los estilos ya aprobados.
- Desde esta revisión, todo texto debe trasladarse de forma completa y literal desde el Word; no se permiten resúmenes, paráfrasis, simplificaciones ni omisiones.

Criterio aprendido:

> El PDF determina la composición visual, pero el Word es la fuente literal del contenido. La necesidad de adaptar un texto a un componente nunca autoriza resumirlo: debe ajustarse la estructura visual para contener el texto completo.

## Primera maquetación del Tema 3 del CF01 de Diseño y desarrollo de chatbots

- `Tema3.vue` se construyó con base en la hoja 5 de `21730211_CF01_DW.pdf` y el texto completo del Word.
- La sección 3.1 utiliza `bloque-texto-g`, `tarjeta-avatar` y `LineaTiempoD` con sus cuatro etapas.
- La sección 3.2 utiliza dos bloques de tarjetas conectadas, dos tablas institucionales y `TabsC` para comparar chatbot y asistente virtual.
- La muestra temporal en inglés de `TabsC` fue reemplazada por el contenido oficial y no quedó duplicada.
- Las tablas 2 y 3 se completaron con todas las filas del documento instruccional.
- Las repeticiones exactas generadas por cuadros de texto superpuestos en el Word se incorporaron una sola vez, sin resumir el contenido legible.

Criterio aprendido:

> Las muestras institucionales sirven únicamente como patrón estructural. Deben sustituirse con el texto completo del Word y retirarse para evitar contenido de demostración o duplicado.

### Revisión de alineación vertical en Tema 3

- En el bloque `.titulo-pastilla` cercano a la fila 125 se añadió `.mb-0` al párrafo.
- La clase elimina el margen inferior predeterminado que desplazaba visualmente el texto, permitiendo que `align-items: center` lo centre respecto al icono.
- No se modificaron el texto, el icono, los colores ni la estructura del bloque.

## Primera maquetación del Tema 4 del CF01 de Diseño y desarrollo de chatbots

- `Tema4.vue` se construyó con base en la hoja 6 del PDF y el texto completo del Word.
- La sección 4.1 presenta las siete ventajas mediante `AcordionA(tipo="b")`, conservando literalmente los textos recuperados del SmartArt del documento.
- La Tabla 4 contiene los ocho sectores, aplicaciones y beneficios del documento instruccional.
- La sección 4.2 presenta los seis indicadores mediante `TabsA`, incluidos sus valores de referencia.
- Se conservaron completos los párrafos narrativos, sin resúmenes ni paráfrasis.

Criterio aprendido:

> El contenido almacenado en diagramas SmartArt del Word también forma parte de la fuente textual y debe extraerse e incorporarse completo en los componentes interactivos.

### Revisión de `TabsA` en Tema 4

- Se eliminó la instancia inicial que agrupaba los seis indicadores y la muestra duplicada agregada como referencia.
- Los indicadores se dividieron en dos componentes `TabsA`, con tres pestañas en cada uno, según la composición del PDF.
- El primer `TabsA` se acompaña de `img7.svg` a la derecha y el segundo de `img8.svg` a la izquierda.
- Se conservan completos los títulos, descripciones y valores de referencia del Word.

### Revisión del ancho de las pestañas en Tema 4

- Los dos `TabsA` incorporan la clase específica `.tema4-tabs`.
- Desde el breakpoint `lg`, la distribución interna cambia de `4/8` —y de `3/9` en XL— a `5/7`.
- El ancho adicional permite mostrar completos los nombres de los indicadores sin modificar el componente instalado ni afectar otros temas.
...
Documento principal usado: fuentes/12210030_CF01_DI.docx
Documento de actividad usado: fuentes/12210030_CF01_AD.docx
Diseño usado: fuentes/12210030_CF01_DW.pdf y fuentes/12210030_CF01_DW.xd
Estado: en progreso
Observaciones: se validaron el documento instruccional, la actividad didáctica y las ocho páginas del PDF de diseño. La paleta Sass y los recursos de portada, introducción y síntesis coinciden con la guía visual. Se configuraron el título, la descripción, el menú de tres temas con trece subtemas, el glosario, las referencias, la introducción, la síntesis y la actividad didáctica de 20 preguntas. El video introductorio aún no dispone de URL o archivo final; el PDF conserva un espacio para video. Las imágenes existentes en src/assets/actividad pertenecen a una base temática anterior y deben reemplazarse o aprobarse antes del cierre visual. La maquetación de Tema1.vue, Tema2.vue y Tema3.vue queda como siguiente etapa.
```

## Maquetación inicial del CF01 de Toma de decisiones en el nivel gerencial

- Bloque revisado: configuración general, introducción, síntesis y actividad didáctica.
- Estado anterior: la configuración y las vistas conservaban textos genéricos o contenido de otros componentes formativos.
- Estructura aplicada: menú sincronizado con los tres temas y trece subtemas del Word; introducción construida con retícula Bootstrap 9/3, destacado institucional e imagen lateral; síntesis con recurso SVG y anexo PDF; cuestionario configurado con 20 preguntas.
- Elementos conservados: paleta institucional, recursos de portada, imágenes de introducción, SVG y PDF de síntesis, rutas existentes y plantilla base.
- Elementos reemplazados: nombre y descripción genéricos, menú de ejemplo, glosario, referencias, textos Lorem ipsum y cuestionario de microcontroladores.
- Pendientes de recurso: URL o archivo del video introductorio e imágenes definitivas de la actividad.
- Siguiente etapa: maquetación visual y textual de `Tema1.vue`, `Tema2.vue` y `Tema3.vue` contra las páginas 3, 4 y 5 del PDF.

## Primera maquetación del Tema 1 del CF01 de Toma de decisiones en el nivel gerencial

- Vista: `src/views/Tema1.vue`.
- Página de referencia: página 3 de `fuentes/12210030_CF01_DW.pdf`.
- Fuente textual: sección 1 de `fuentes/12210030_CF01_DI.docx`, desde “Naturaleza, proceso y condiciones de la decisión gerencial” hasta el cierre del apartado 1.5.
- Contenido: se conservaron completos los textos, ejemplos, citas y cinco subapartados del Word; el latinismo `decisio` se mantuvo en cursiva.
- Recursos: se utilizaron de forma completa `img1.png` a `img57.png`/`.svg` de `src/assets/curso/temas/t1/` y todas las imágenes decorativas usan `alt=""`.
- Componentes institucionales: `tarjeta-avatar-b`, `tarjeta-avatar`, `bloque-texto-g`, `SlyderF`, `AcordionA`, `LineaTiempoD`, `.tarjeta--boton`, `.tarjeta--container`, lista institucional y títulos de segundo nivel con anclajes.
- Distribución: la apertura usa retícula 4/8; los atributos se presentan en `SlyderF`; las decisiones por estructuración en `AcordionA`; los niveles temporales y rasgos culturales en tarjetas conectadas; las ocho etapas se dividen en dos `LineaTiempoD`; y las capacidades comunicativas usan tarjetas botón con cambio de icono al pasar el puntero.
- Estilos: los ajustes particulares del Tema 1 se añadieron exclusivamente a `src/styles/_custom.sass`, reutilizando la paleta definida en `_variables.sass`.
- Validación: los cinco hashes del menú existen en la vista, las 57 imágenes están referenciadas, el contenido largo del Word está presente y `npm run build` finaliza correctamente.
...
## Incorporación inicial del contenido de los Temas 1 al 6 del CF02

- Se trasladó a `Tema1.vue` hasta `Tema6.vue` el desarrollo temático completo de `13530530_CF02_DI.docx`.
- Se conservaron el orden oficial, los párrafos, los títulos principales y los 21 apartados definidos en la tabla de contenidos.
- Los `hash` del menú se mantuvieron sincronizados con los 21 identificadores de las vistas.
- Las siete tablas del documento se construyeron con el componente institucional `.tabla-a.color-acento-contenido` y títulos `.titulo-sexto`.
- Los rótulos internos se representaron inicialmente con `h5`; la composición visual definitiva debe revisarse posteriormente bloque por bloque contra el PDF.
- Se eliminaron duplicados exactos producidos por cuadros de texto repetidos en la estructura interna del Word.
- Se agregó `scripts/generate-theme-content.ps1` como mecanismo reproducible de extracción; el script se conserva en UTF-8 con BOM por compatibilidad con Windows PowerShell y genera las vistas en UTF-8 sin BOM.
- `npm run build` finalizó correctamente después de incorporar el contenido.

Criterio aprendido:

> Cuando el Word contiene un desarrollo extenso distribuido en varios temas, primero se debe garantizar la incorporación completa y ordenada del contenido, las tablas y los anclajes. Después se realiza la maquetación visual bloque por bloque con base en el PDF y los recursos exportados.

## Maquetación de la introducción del CF02

- `Introduccion.vue` se maquetó con base en la página 2 de `13530530_CF02_DW.pdf`.
- La apertura utiliza `intro/img1.png` como imagen panorámica y conserva el primer párrafo como bloque independiente.
- El contexto formativo se presenta en una tarjeta responsiva con `intro/img2.png`, título interno `h4`, texto oficial y botón hacia la guía del Ministerio de Educación Nacional indicada en el Word.
- El párrafo central se destaca mediante un cajón secundario centrado a diez columnas.
- El bloque siguiente utiliza `intro/img3.svg` con su tamaño original de `84 × 84 px` y texto alineado en una fila responsiva.
- Se conservó el componente de video existente y se ajustó su título accesible.
- Las tres imágenes usan `alt=""` porque no corresponden a figuras numeradas.
- Los estilos decorativos específicos se ubicaron en `_custom.sass`; se eliminó el bloque `<style>` vacío de la vista.
- `npm run build` finalizó correctamente.

## Tema 1 del CF02: bloque anterior a 1.1

- Se maquetó exclusivamente el bloque comprendido entre el título `Diversidad` y el apartado `1.1 Concepto`, con base en la parte inicial de la hoja 3 del PDF.
- La estructura utiliza una fila responsiva `4/8`: `t1/img1.png` en la columna izquierda y los dos primeros párrafos en la derecha.
- El segundo párrafo se presenta mediante `.cajon.color-primario.p-4` y termina con `p.mb-0`.
- El tercer párrafo ocupa todo el ancho y utiliza `.mb-0` porque está seguido inmediatamente por `Separador`.
- `img1.png` usa `alt=""` por no corresponder a una figura numerada.
- No se agregó CSS personalizado; la retícula, el cajón y las utilidades institucionales resuelven el bloque.

## Tema 1 del CF02: sección 1.1

- Se maquetó exclusivamente el contenido de `1.1 Concepto`, conservando sin cambios el bloque anterior y las secciones 1.2 a 1.5.
- El primer párrafo utiliza `bloque-texto-g--inverso` con `t1/img2.png` como imagen de fondo ubicada al costado derecho.
- El segundo párrafo se presenta mediante `.cajon.color-secundario.p-4`, centrado en diez columnas.
- El cierre se organiza en una fila responsiva con `t1/img3.svg` de 84 px y el texto en la columna restante.
- `img2.png` se carga mediante `require_src`; `img3.svg` utiliza `alt=""` por no ser una figura numerada.
- La fila final usa `.mb-0` porque está seguida por `Separador`.
- No fue necesario agregar CSS personalizado.

## Tema 1 del CF02: sección 1.3

- Se maquetó exclusivamente `1.3 Tipos`, siguiendo la composición correspondiente de la hoja 3 del PDF.
- La apertura utiliza `bloque-texto-g--inverso` con `t1/img7.png` y el texto superpuesto sobre el panel blanco.
- La tabla conserva el componente institucional y aplica `.tema1-tabla` para diferenciar la cabecera y las filas alternadas.
- La figura incorpora `t1/img8.svg` con una descripción alternativa de los seis tipos de diversidad representados.
- El texto de cierre se presenta sobre el fondo violeta claro `.tema1-puente-fondo`.
- Las etapas `Antes`, `Durante` y `Después` se organizaron en tres tarjetas responsivas con `t1/img9.svg`, `t1/img10.svg` y `t1/img11.svg`.
- `npm run build` finalizó correctamente.

## Tema 1 del CF02: sección 1.2

- Se maquetó exclusivamente `1.2 Características`, manteniendo sin cambios las secciones ya aprobadas y los apartados posteriores.
- El primer párrafo se conserva como introducción independiente.
- El segundo párrafo reutiliza el patrón `.tema1-apertura` en una retícula `5/7`, con `t1/img4.png`, `t1/img5.svg` y panel violeta claro.
- Las seis características se implementaron con `LineaTiempoD.color-acento-botones`; cada elemento es hijo directo y declara `numero` y `titulo`.
- El componente ocupa siete columnas y `t1/img6.png` las cinco columnas restantes.
- El párrafo final utiliza `.mb-0` antes de `Separador`.
- Todas las imágenes usan `alt=""` porque no son figuras numeradas.
- No fue necesario agregar CSS personalizado.
### Maquetación de Tema 5: herramientas de software para el desarrollo de chatbots

- Se maquetó la hoja 7 del PDF con el contenido completo del documento fuente, sin resúmenes ni paráfrasis.
- La sección 5.1 presenta la definición mediante `bloque-texto-g` y distribuye los tres tipos de plataformas en un `SlyderA(tipo="b")`, dentro de `.BGF01.px-5.p-5 > .bgwhite.p-4`.
- La tabla 5 conserva las ocho plataformas, sus tipos y sus características tal como aparecen en el Word.
- La sección 5.2 organiza los ocho criterios de selección en `LineaTiempoC`, dentro de `.BGF02.px-5.p-5`, y conserva los textos complementarios sobre pruebas, portabilidad, integraciones y documentación.
- La tabla 6 conserva completos los cinco escenarios, las plataformas sugeridas y sus justificaciones.

> En los temas siguientes se mantiene como regla obligatoria trasladar íntegramente el texto del Word. El PDF determina la composición visual y los componentes institucionales, pero no autoriza resumir, parafrasear ni omitir contenido.

### Maquetación de Tema 6: frameworks de diseño conversacional

- Se maquetó la hoja 8 del PDF conservando el contenido completo del Word.
- Las cinco fases de la metodología se presentan en `SlyderF` y los cinco métodos de diseño centrado en el usuario en `LineaTiempoD`.
- El principio de conversación cooperativa se presenta mediante `bloque-texto-g--inverso`.
- La Figura 2 cuenta con versiones responsivas para escritorio y móvil, y los tres escenarios de manejo de situaciones inesperadas se distribuyen en tarjetas conectadas.
- Se conservaron los textos finales sobre el diseñador conversacional, la aplicación de las metodologías y la introducción al pódcast.

### Maquetación de Tema 7: alistamiento de datos

- Se maquetó la hoja 9 del PDF con el texto íntegro del Word, sin resúmenes ni paráfrasis.
- La apertura utiliza `bloque-texto-g`; la sección 7.1 presenta las cuatro fuentes de información en `TabsC`; y la sección 7.2 organiza intención, entidad y frase de entrenamiento en `AcordionA(tipo="b")`.
- Se conservaron completos los ejemplos y todos los párrafos sobre preparación, actualización, privacidad, seguridad y diversidad lingüística.
- Las cursivas se trasladaron según el formato del Word: `chatbot`, `chatbots`, `chats` y `clustering`.

> Además de validar la igualdad textual, cada tema debe revisar el formato de los fragmentos del Word. Las cursivas de la fuente también forman parte del contenido que debe conservarse en la maquetación.

### Maquetación de Tema 2: modelos clásicos de decisión

- Se maquetó la página 4 del PDF con el contenido completo de la sección 2 del Word, sin resúmenes ni paráfrasis.
- La apertura presenta los cuatro rasgos de interpretación de los modelos mediante `SlyderF`.
- Las secciones 2.1 y 2.2 utilizan `AcordionA` para los supuestos del modelo racional y los casos de racionalidad limitada.
- Los ejemplos empresariales de las secciones 2.1, 2.2 y 2.4 se organizaron con `tarjeta-avatar-b` y tarjetas de avatar institucionales.
- La sección 2.3 incorpora la infografía institucional tipo A mediante `ImagenInfografica`, con cuatro modales sobre los conceptos de teoría de juegos y composiciones independientes para escritorio y móvil. También conserva las cuatro tarjetas interactivas con cambio de icono para los casos empresariales.
- Se conservaron las cursivas del Word para `Administrative Behavior`, `Models of Man`, `bounded rationality`, `satisficing`, `satisfying` y `sufficing`.
- Se eliminaron solo las repeticiones técnicas producidas por los cuadros de texto destacados del Word; cada enunciado permanece una vez en su bloque visual correspondiente.
- Se utilizaron los 48 recursos gráficos disponibles para el tema y se mantuvieron sincronizados los anclajes `2.1` a `2.4`.
- `npm run lint` y `npm run build` finalizaron correctamente.

### Maquetación de Tema 3: enfoques contemporáneos en la toma de decisiones

- Se maquetó la hoja 5 del PDF con el contenido completo de la sección 3 del Word, sin resúmenes ni paráfrasis.
- La apertura conserva los cuatro rasgos de los enfoques contemporáneos en tarjetas conectadas.
- La sección 3.1 presenta las estrategias frente a la incertidumbre mediante tarjetas de avatar, un bloque destacado, `AcordionA` y una secuencia de preguntas A–D.
- La sección 3.2 incorpora `ImagenInfografica`, correspondiente a la infografía institucional tipo A, con los seis sesgos cognitivos y sus textos interactivos. Se implementaron composiciones independientes para escritorio y móvil.
- Los textos de los seis modales se recuperaron de los comentarios del Word, donde estaban especificados como recurso interactivo.
- Las secciones 3.3 y 3.4 conservan los componentes sobre decisiones basadas en datos, apoyo de la inteligencia artificial, riesgos, casos y pasos de adopción responsable.
- Se conservaron en cursiva los títulos y extranjerismos `Risk, Uncertainty and Profit`, `Thinking, Fast and Slow`, `Data-Driven Decision Making` y `pre-mortem`.
- Se utilizaron los 53 recursos gráficos disponibles para el tema y se mantuvieron sincronizados los anclajes `3.1` a `3.4`.
- El bloque del pódcast quedó maquetado, pero el proyecto no contiene todavía el archivo de audio correspondiente.
- `npm run lint` y `npm run build` finalizaron correctamente.

### Maquetación de Tema 8: técnicas para el desarrollo de flujos de conversación

- Se maquetó la hoja 10 del PDF con el contenido íntegro del Word.
- La sección 8.1 utiliza la muestra institucional `PasosB` para voz, tono, personalidad y matriz de valores.
- La sección 8.2 inicia con `bloque-texto-g--inverso` y presenta las cuatro variaciones de consulta de horario mediante `tarjeta--boton`.
- Se incorporaron completos los apartados de accesibilidad, manejo de errores, recursos para la experiencia y mejora continua.
- La tabla 7 conserva sus seis tipos de error, descripciones y estrategias recomendadas.
- Se respetaron las cursivas del Word para `chatbot`, `chatbots` y `marketing`.

## Configuración de la estructura general del CF02 de Toma de decisiones en el nivel gerencial

- Se utilizó `fuentes/12210030_CF02_DI.docx` como fuente oficial para el nombre, la descripción, la tabla de contenidos, el glosario y las referencias bibliográficas.
- Se configuró el componente **Herramientas de evaluación y seguimiento en la estructura organizacional** en `src/config/titulo.js` y `src/config/global.js`.
- El menú principal quedó organizado en cinco temas y veintiún subtemas, con sus respectivos `hash`.
- Se conservaron las vistas `Tema1.vue` a `Tema3.vue`, se actualizaron sus títulos y anclajes, y se crearon `Tema4.vue` y `Tema5.vue`.
- El enrutador incorpora las rutas de los cinco temas.
- Se incorporaron los doce términos del glosario y las diez referencias bibliográficas del documento instruccional.
- Para el Tema 3 se utilizó el título oficial del Word, **Seguimiento de la decisión en la estructura organizacional**, debido a que el PDF de diseño conserva el rótulo inconsistente **Modelos clásicos de decisión**.
- Se verificó la correspondencia entre los veintiún `hash` del menú y los veintiún identificadores de las vistas.
- `npm run build` finalizó correctamente. Se mantiene una advertencia preexistente y no bloqueante sobre la ruta `assets/bullets/puntero.svg`.

Criterio aprendido:

> Al iniciar un nuevo componente, la tabla de contenidos y los títulos oficiales deben provenir del documento instruccional. Si el PDF conserva un rótulo de otro componente, se usa el texto del Word y se registra la inconsistencia antes de iniciar la maquetación visual.

## Maquetación de la síntesis del CF02 de Toma de decisiones en el nivel gerencial

- Se incorporaron en `Sintesis.vue` los dos párrafos oficiales de la sección Síntesis de `fuentes/12210030_CF02_DI.docx`.
- Se conservaron `src/assets/curso/sintesis.svg` y `public/downloads/Sintesis.pdf`, porque ambos corresponden al mapa conceptual y al diseño aprobados para este componente.
- El mapa conceptual organiza los cinco ejes y los veintiún subtemas del componente.
- Se reemplazó el texto alternativo genérico por una descripción que comunica la estructura esencial del mapa.
- El icono decorativo del anexo utiliza `alt=""`.
- Se retiraron las clases sin definición `.bgs` y `.brad`, así como el bloque `<style>` vacío.

Criterio aprendido:

> Cuando la síntesis gráfica y el PDF descargable ya corresponden al componente actual, deben conservarse; la vista se ajusta mediante el contenido oficial, los márgenes institucionales y una descripción alternativa suficiente.

## Configuración de la actividad didáctica del CF02 de Toma de decisiones en el nivel gerencial

- Se reemplazó el cuestionario heredado de microcontroladores por las veinte preguntas oficiales de `fuentes/12210030_CF02_AD.docx`.
- Se incorporaron el nombre, el objetivo, la instrucción, las ochenta opciones, las retroalimentaciones y los mensajes finales de aprobación y no aprobación.
- Cada pregunta conserva `barajarRespuestas: true` y tiene exactamente una respuesta correcta.
- `barajarPreguntas: true` permite que el controlador institucional seleccione diez de las veinte preguntas en cada intento.
- Se conservó el umbral institucional del 70 % definido por el componente.
- Se eliminó la actividad alternativa de completar frases porque no pertenece al documento oficial.
- Se conservaron las cursivas de *Design Thinking* mediante la etiqueta `<em>`.
- Se reutilizaron las ilustraciones gerenciales disponibles y se excluyó `imagen3.png`, porque corresponde a un circuito electrónico del contenido heredado.
- La validación confirmó veinte preguntas, cuatro opciones por pregunta y una única respuesta correcta en cada caso.
- Los archivos modificados cumplen el formato de Prettier y `npm run build` finaliza correctamente. Se mantiene la advertencia preexistente y no bloqueante sobre `assets/bullets/puntero.svg`.

Criterio aprendido:

> La actividad debe construirse desde su documento específico y validarse como estructura de datos: cantidad de preguntas, cantidad de opciones, una sola respuesta correcta, retroalimentaciones completas y mensajes finales. Los recursos gráficos heredados que contradigan el tema no deben utilizarse.

## Maquetación de la introducción del CF02 de Toma de decisiones en el nivel gerencial

- `Introduccion.vue` se maquetó con base en la hoja 2 de `fuentes/12210030_CF02_DW.pdf` y los cinco párrafos oficiales de la introducción de `fuentes/12210030_CF02_DI.docx`.
- Se utilizó como referencia estructural la introducción previamente aprobada del CF01 ubicada en `D:/Sena/Sena2026/Github/Septiembre/12210030_CF01_TOMA_DECISIONES_NIVEL_GERENCIAL`.
- La apertura usa una retícula responsiva `3/9`: `intro/img1.png` a la izquierda y el primer párrafo junto con el destacado a la derecha.
- El segundo párrafo se presenta en `.introduccion-destacado.p-4` con `intro/img2.svg`, reutilizando las clases existentes de `_custom.sass`.
- El tercer párrafo ocupa todo el ancho como bloque independiente.
- El cuarto párrafo utiliza un cajón secundario junto a `intro/img3.svg` en una retícula `9/3`.
- El quinto párrafo introduce el video oficial. Se conservó sin cambios la URL previamente enlazada y se normalizó el título accesible y el rótulo `Video 1.`.
- Las tres imágenes usan `alt=""` porque son recursos decorativos y no corresponden a figuras numeradas.
- No fue necesario agregar CSS: la maquetación utiliza las clases de introducción ya definidas y componentes institucionales.
- El bloque `<style>` vacío y el objeto `data` sin uso fueron eliminados.

Criterio aprendido:

> El contenido del Word debe incorporarse completo, sin resúmenes, paráfrasis ni omisiones. Las cursivas y negrillas también forman parte del contenido y deben trasladarse. Cuando el nuevo componente repite una composición aprobada del CF anterior, se reutilizan su estructura y sus clases existentes, ajustando únicamente el orden de columnas, los recursos y los textos indicados por el nuevo diseño.

### Revisión de la fila final de la introducción

- El párrafo que recomienda acceder al video se trasladó al `.col-lg-9` de la fila que contiene el cajón secundario y `intro/img3.svg`.
- El párrafo utiliza `.mt-4.mb-0`, mientras la separación respecto al video se mantiene mediante `.mb-4` en la fila.
- Se retiró `.h-100` del cajón para permitir que el párrafo comparta la columna sin exceder su altura.
- Se conservaron sin cambios el contenido, los colores, las imágenes y la URL del video.

Criterio aprendido:

> Cuando una imagen lateral acompaña un cajón y el texto que introduce el bloque siguiente, ambos textos deben permanecer en la misma columna de la fila para que la altura de la imagen corresponda al conjunto completo.

### Regla responsive para la alternancia de recursos visuales

- En dispositivos pequeños no deben quedar dos imágenes consecutivas; esta regla también considera los videos como recursos visuales.
- Cuando sea necesario alternar el contenido, se deben utilizar las utilidades `order-*` de Bootstrap en las columnas, conservando en escritorio el orden definido por la composición del PDF.
- En el bloque final de la introducción, la columna de texto utiliza `.order-2.order-lg-1` y la columna de la figura utiliza `.mb-4.mb-lg-0.order-1.order-lg-2`.

## Maquetación del Tema 1 del CF02

- `Tema1.vue` se desarrolló con la composición de la hoja 3 de `fuentes/12210030_CF02_DW.pdf` y el contenido completo del Tema 1 de `fuentes/12210030_CF02_DI.docx`.
- Se conservan íntegros los textos del Word, incluidos los títulos, los casos colombianos, las frases destacadas, los listados y el contenido de la Tabla 1.
- La sección 1.1 utiliza `ImagenInfografica` para los cuatro cruces de la matriz FODA y una versión gráfica vertical en dispositivos pequeños.
- La sección 1.2 utiliza `SlyderF` para las seis dimensiones PESTEL y tarjetas conectadas para sus tres casos de aplicación.
- La sección 1.3 utiliza `LineaTiempoD` para presentar los cuatro rasgos del grupo nominal y del método Delphi.
- La sección 1.4 incorpora la Figura 1 con versiones horizontal y vertical, además de `ImagenInfografica` para los siete factores del modelo McKinsey 7S.
- La sección 1.5 utiliza `LineaTiempoD` para las cuatro perspectivas del cuadro de mando y reproduce la Tabla 1 con sus cuatro encabezados y sus cuatro celdas de contenido.
- Las figuras decorativas tienen `alt=""`; la Figura 1 y las infografías cuentan con textos alternativos descriptivos.
- En los bloques responsive se alternó el orden de imagen y contenido para evitar dos recursos visuales consecutivos, considerando también las infografías como recursos visuales.
- El archivo cumple el formato de Prettier y `npm run build` finaliza correctamente. Se mantiene la advertencia preexistente y no bloqueante sobre `assets/bullets/puntero.svg`.

Criterio aprendido:

> El PDF orienta la composición y el componente institucional que corresponde a cada bloque, mientras el Word conserva la autoridad sobre el contenido. Los textos de tablas, infografías y componentes interactivos también deben extraerse completos del documento y de sus anotaciones, sin resumirlos ni sustituirlos por el texto visible en la referencia gráfica.

## Maquetación del Tema 2 del CF02

- `Tema2.vue` se desarrolló con la composición de la hoja 4 de `fuentes/12210030_CF02_DW.pdf` y el contenido completo del Tema 2 de `fuentes/12210030_CF02_DI.docx`.
- Se conservaron íntegros los textos del Word, incluidos los conceptos, los casos colombianos, las frases destacadas y las secuencias de aplicación.
- La sección 2.1 presenta los cuatro conceptos del análisis costo-beneficio mediante tarjetas conectadas, los tres casos mediante una lista con separadores y las seis fases en un listado numerado.
- La sección 2.2 incorpora la Figura 2 en versiones horizontal y vertical, junto con tres tarjetas de casos y bloques de transición que reproducen la composición del PDF.
- La sección 2.3 incorpora la Figura 3 en versiones horizontal y vertical, los tres casos en una lista con separadores y los cinco pasos de construcción de la matriz.
- Las expresiones del documento fuente se conservaron literalmente, incluso cuando indican “en los cuatro casos” después de presentar tres casos.
- Los recursos decorativos utilizan `alt=""`; las Figuras 2 y 3 cuentan con textos alternativos que explican la información esencial de cada gráfico.
- Se aplicó la regla responsive de alternancia mediante `order-*` en los bloques donde la imagen aparece a la derecha en escritorio.
- Se añadieron a `_custom.sass` las cuatro variantes cromáticas y el tamaño de icono de las tarjetas conectadas del Tema 2.
- `npm run build` finaliza correctamente. Se mantienen las advertencias preexistentes y no bloqueantes sobre `assets/bullets/puntero.svg` y el tamaño del paquete principal.

Criterio aprendido:

> Cuando el Word contiene una formulación que parece inconsistente con la cantidad de elementos maquetados, se conserva literalmente. La fidelidad editorial tiene prioridad y cualquier corrección debe provenir de una instrucción expresa del equipo de contenido.

## Maquetación del Tema 3 del CF02

- `Tema3.vue` se desarrolló con la composición de la hoja 5 de `fuentes/12210030_CF02_DW.pdf` y el contenido completo del Tema 3 de `fuentes/12210030_CF02_DI.docx`.
- Se utilizó el título oficial del Word, **Seguimiento de la decisión en la estructura organizacional**, aunque el PDF conserva el rótulo inconsistente **Modelos clásicos de decisión**.
- La sección 3.1 incorpora la Tabla 2 completa, las cuatro clases de indicador, los tres casos empresariales y la secuencia de cuatro pasos.
- La sección 3.2 utiliza `SlyderF` para los cuatro componentes del almacenamiento de información y conserva los tres casos de aplicación.
- La sección 3.3 utiliza `LineaTiempoD` para los cinco formatos del proceso decisional y tarjetas institucionales para el anexo y los cuatro recursos oficiales.
- La sección 3.4 utiliza `AcordionA` para los tres casos de retroalimentación y ajuste.
- La sección 3.5 presenta las cuatro responsabilidades del líder y los tres casos de implementación mediante los componentes gráficos aprobados.
- Se preservaron literalmente las expresiones “en los cuatro casos” aunque las fuentes presenten tres casos.
- Los enlaces externos de la sección 3.3 se tomaron de las anotaciones del Word. La ruta local `/downloads/Anexo_01_Formatos_Decision.pdf` quedó preparada, pero el archivo fuente aún no está disponible en el proyecto.
- Se añadieron las clases de `tema3-slyder` y `tema3-slyder__icono` para reproducir el patrón aprobado del CF01.
- `npm run build` finaliza correctamente. Se mantienen las advertencias preexistentes sobre `assets/bullets/puntero.svg` y el tamaño del paquete principal.
